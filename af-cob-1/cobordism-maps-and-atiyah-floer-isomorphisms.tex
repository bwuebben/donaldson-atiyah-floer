...
framed instanton homology. The analytic input consists of two new results
...
\cite{DFL}, with the invariant perturbations of Section~\ref{sec:models};
we do not identify them with the isomorphism of \cite{DFL} for general data, and the
proof of Theorem~\ref{thm:A} does not require such an identification
(Remark~\ref{rem:dfl-map}). The transitive system of Theorem~\ref{thm:A} is constructed
from the gauge side; we do not compare it with the Cerf-theoretic identifications
of \cite{WW-fft}.
...
supported in the interior of $C$, small enough that the perturbed flat
...
(\cite{DFL} call these perturbations cylinder functions.) The universal restriction map from pairs (perturbation, flat
...
\item metrics on $C_0$, $C_1$ that are products near the boundary and
  agree on $\Sigma_h$ and on a collar of $T^2$ carrying the flat metric fixed
...
(Remark~\ref{rem:dfl-map}). The gauge complex
...
construction because the supports of a cylinder function and of its translate
by $\alpha_M$ are disjoint. There are finitely many energy levels below the
...
The map $N_p$ is defined by the construction of \cite[Section~3.2]{DFL},
with the end perturbations of Proposition~\ref{prop:input-functions}
added and the secondary perturbations restricted to the class invariant
under $\phi_M$. We do not prove that $N_p$ agrees with the isomorphism of
\cite[Theorem~3.29]{DFL}, which is defined with unrestricted secondary
perturbations and without end perturbations; DFL regularize the gauge complex
by perturbing the primary cylinder functions \cite[Lemma~2.56]{DFL}, which is
not available for the data used in the two-handle comparison, where the
primary perturbation of the middle compression body must vanish. The
end perturbations of Proposition~\ref{prop:input-functions} replace that
step, and the proof of Theorem~\ref{thm:mixed-iso} does not use
\cite[Lemma~2.56]{DFL}. The proof of
Theorem~\ref{thm:A} uses only properties (i) and (ii).
\end{remark}
...
neighbourhood of $L(U_i)$ that is small relative to $t$. In
...
generator $q^*$. A small further perturbation of $R$ supported away from the
critical set keeps it a height Lagrangian at height $t$.
...
$\mathcal U(t)$ be a neighbourhood so small that every choice of primary data in it and its
reflection are lower Lagrangians small relative to $t$ in the sense of
Section~\ref{subsec:folded}; the condition is symmetric under reflection,
...
\emph{The admissible neighbourhoods.} For each splitting, metric data and finite
family $F$ as in Definition~\ref{def:presentation}, fix once and for all a height
$t_F$ below the threshold of Proposition~\ref{prop:small-primary}, and let
$\mathcal U(F)$ be the neighbourhood $\mathcal U'(t_F)$ of
Proposition~\ref{prop:duality}. These propositions use
...
$r$ with the metric of $r$. Choose first the heights $t_F$ of Proposition~\ref{prop:small-primary} for the splittings and metric data of
$C\circ p$ and of $r$, then $t$ and the two-handle height $t'$ so small that
...
$L(V_1)$ and to $L_{t'h_Q}(C_2)$ and missing their intersection; this is a
generic condition. Put
\begin{itemize}[leftmargin=2em]
\item $(C\circ p)'$: the presentation $C\circ p$ with primary data $(R_0',0)$;
\item $r'$: the presentation $r$ with primary data $(R_0',t'h_Q)$.
\end{itemize}
Both are admissible. The subordinate triple of the cancelling two-handle is
$(C_0,C_1,C_2)=(V_0,V_1,C_2)$ with primary data $(R_0',0,t'h_Q)$, which
...
The triple of $W_2'$ is $(V_0,C_2,V_1)$ with primary data $(R_0'',0,t''h_Q)$,
chosen in the same way, and
\[
D_{p,r}=V_C\circ \mathrm{cont}_S\bigl((C\circ p)'',C\circ p\bigr)\circ S_{W_2'}\circ
\mathrm{cont}_S(r,r''),
\]
where $r''$ carries $(R_0'',0)$ and $(C\circ p)''$ carries $(R_0'',t''h_Q)$.
