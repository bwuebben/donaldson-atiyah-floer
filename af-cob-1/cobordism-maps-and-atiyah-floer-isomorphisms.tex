...
ends appear along which the matching surface runs to infinity, with a Morse--Bott family of stationary solutions. The index-zero
counts are unchanged, and a single affine relation between energy
and index holds at every scale.
...
proof proceeds by a closed cut at a fixed bridge width, a homotopy
of the width and a completion of the torus region
(Section~\ref{sec:torus}, plan of the proof). On the plane,
half-plane and straight strips, the linearized operator at the odd
flat torus has an inverse bounded by $\mu_T^{-1}$, independently
of strip width (Proposition~\ref{prop:flat-fibre}). Small-energy
regularity up to the flat face follows by placing the connection
in a Neumann slice and absorbing the nonlinear coexact boundary
trace (Theorem~\ref{thm:flat-boundary-regularity}). Solutions decay
exponentially along the completed tail
(Proposition~\ref{prop:tail-decay}).
...
$\mathfrak h\in\mathcal U(F)$ primary data with transverse Lagrangians. Then there are
arbitrarily small split additions $g$, each lying in the span of a finite family $G$ of
cylinder functions with loops in
$\operatorname{int}C_0^\circ\sqcup\operatorname{int}C_1^\circ$, such that
$\mathrm{CS}_{\mathfrak h+g}$ has the same critical
...
therefore admissible. The family $G$ may depend on the required smallness of $g$, and its loops
may be taken in $\operatorname{int}C_0^\circ$ alone. The metric data are not changed.
\end{theorem}
...
\item\label{H:regular} \emph{Component regularity.} Each piece is regular
  ...
  one.
\item\label{H:support} \emph{Supports.} The autonomous primary and end
  perturbations are fixed on the ends and on the corresponding product
  necks. The additional secondary perturbations, the nonautonomous
  cutoff changes, and the parameter derivatives have their output and
  entire loop-dependence regions in a common compact set away from
  the inserted necks. This compact set is fixed after the finite
  selection of secondary directions and before neck lengths vary.
...
Choose $e'=\operatorname{Ad}_{\exp\eta}e$ with a nonzero bump $\eta$
supported where $\psi=1$ and with arbitrarily small
$\|d\eta\|_{L^2}$. Then
$\|d_\alpha(2\pi\psi e')\|_{L^2}\le
2\pi\|d\psi\|_{L^2}+C\|d\eta\|_{L^2}$.
A logarithmic cutoff makes the first term small as well. Thus the
two distinct forms can lie in any $L^2$ ball; rescaling a fixed
profile alone would not decrease its surface Dirichlet energy.
By the Sobolev
...
fixed autonomous primary and end terms have uniform bounds on unit
patches. The additional secondary terms and nonautonomous parameter
variations have compact dependence regions as in \ref{H:support}. Hence the
...
higher derivatives uniformly bounded by the half-disk estimates of
\cite[Lemma~C.3]{RS-strips}, applied at each boundary with its own
Lagrangian. The linear elliptic equation for the time derivative
then upgrades the derivative decay of \cite[Proposition~2.3.1(ii)]{Simcevic}
to decay of all orders on smaller unit patches;
...
piece having an end. This contradicts $\|\xi_n\|_{L^2}=1$.
Write $C_P$ for this uniform scalar Poincar\'e constant and, in
addition to \eqref{eq:neck-weight}, take $\delta C_P<1/2$.
Choose the smoothing below with $|dW|\le1$. The bilinear form
$\langle d_p(e^{-\delta W}V),d_p(e^{\delta W}U)\rangle_{L^2}$, where $W$ is a
smoothing of the end coordinate of the mixed ends, satisfies
$B(U,U)=\|d_pU\|^2-\delta^2\|(dW)U\|^2\ge
(1-\delta^2C_P^2)\|d_pU\|^2$ and is coercive on $L^2_1$. The
...
For the regularity assertion, distinguish the output support of a
holonomy term from the region traversed by its loops. On each gauge
component choose an open output region $O$ containing the outputs of
all fixed perturbation derivatives. It may extend down original ends.
Every component of $O$ is joined through permitted loop-dependence
regions to an original end whose limiting connection is flat on an
odd surface collar and on which the lifting characters vanish.
Prospective directions have arbitrary compact output support in $O$
and compact loop dependence in these regions. On a torus joined
problem take the union of the two such regions on its two sides;
the loops need not cross the inserted torus bridge. The compact
support set for the eventual finite collection is chosen afterwards.
...
\item For a gauge or mixed problem with the output and detection
  regions just specified, the linearized operator together with all
  permitted invariant holonomy directions is onto at every solution.
  On purely symplectic components use the ordinary permitted
  almost-complex and Hamiltonian variations instead.
\item Under the compactness and gluing hypotheses of
  Theorems~\ref{thm:neck} and~\ref{thm:family}, generic small invariant
  perturbations make the required low-index spaces regular.
  Finitely many directions are selected at each index, relative to
  already regular broken strata and parameter boundary regions.
  Their common compact dependence region is then fixed before
  neck lengths are varied.
\end{enumerate}
\end{theorem}
...
(ii) Write the full gauge-fixed derivative at a solution as
\[
D(a,v)=\bigl(d_A^*a,(d_A^++K_A)a,D_uv\bigr),
\]
where $K_A$ includes all primary and secondary holonomy derivatives.
Let $(\mu,\xi,z)$ annihilate this operator and all prospective
columns. A Neumann gauge variation $(d_Af,0)$ is an admissible
source; its surface harmonic class is zero. Gauge covariance at a
solution gives $(d_A^++K_A)d_Af=0$, so
$\langle\mu,d_A^*d_Af\rangle=0$.
The scalar Neumann operator is onto at each such solution. Indeed
its end operators have positive scalar gaps; on a completed torus
end use the odd-fibre gap. The compact-remainder argument therefore
makes it Fredholm at weight zero. Integration by parts identifies
its kernel with decaying parallel sections, which vanish by
irreducibility. Self-adjointness gives index zero, and sufficiently
small weights preserve that index and introduce no kernel. This
pointwise argument precedes any regularity or finite-set assertion
for the mixed solution spaces. Hence $\mu=0$.

At the limiting odd reference surface, Lemma~\ref{lem:invariant-span}
provides finitely many holonomies spanning the adjoint fibre over
$\mathbb R$. Move their loops slightly into the protected flat
collar and then to a sufficiently far, finite slice of the original
end. Convergence to the limiting connection preserves this span.
Conjugate the loops by a common path to any prospective output ball
in $O$. The path and its reverse cancel in every lifting character,
so these loops still have zero character. Thicken them smoothly to
holonomy families. Their self-dual output forms can be chosen
arbitrarily in the ball; the real coefficients expressing a target
vector in the spanning holonomies are computed at this solution
and frozen as perturbation data. Pairing with all such columns
therefore gives $\xi|_O=0$.

All fixed nonlocal derivatives have output in $O$. Consequently,
for every test variation $a$,
$\langle K_A^*\xi,a\rangle=\langle\xi,K_Aa\rangle=0$.
Thus the remaining interior adjoint equation is genuinely local:
$d_A^{+,*}\xi=0$. Applying $d_A^+$ gives an elliptic second-order
equation with scalar principal symbol. Unique continuation
\cite{Aronszajn} implies $\xi=0$ on each connected gauge component.
This argument eliminates the nonlocal adjoints before using unique
continuation; projection to a gauge slice alone would not do so.

On an open matching arc choose an arbitrary section trace, represent
its components by harmonic forms on the matching surfaces, and
extend them independently into the gauge collars with zero normal
component. These are admissible coupled variations. Since both
gauge cokernel components vanish, their Green pairing forces the
section adjoint to have zero boundary trace on that arc. Mixed
adjoint regularity supplies this trace. Extension by zero across a
smaller arc, followed by unique continuation for the real
Cauchy--Riemann equation, gives $z=0$. Purely symplectic components
are regularized separately by their usual $J$ and Hamiltonian
variations \cite[Theorem~3.9]{WW-relative}; they do not use holonomy
columns. The ordinary gauge pieces here have framed reference ends
with the same protected odd collar. Closed gauge factors evaluated
in the excision theorem retain the source results of
\cite[Section~5.3]{KM-unknot}.

(iii) At a fixed solution finitely many columns suffice because the
cokernel is finite dimensional. At each index, remove neighbourhoods
of the already regular broken strata and of fixed regular parameter
faces. Compactness of the remaining locus gives a finite collection
of columns by a finite cover. Parameter cutoffs implement relative
choices. Sard--Smale then gives a generic small regular choice.
Only now take a compact set containing the output supports and the
entire loop images of these columns, and place the original-end
truncations farther out. This set is independent of subsequently
inserted lengths. Each column is individually invariant, so no
averaging or loss of span occurs.
\end{proof}
...
We will use the following quantitative consequence of fixed-geometry
mixed regularity. The energy is measured on a larger patch than
the one on which derivatives are estimated.

\begin{lemma}[Quantitative local seam estimate]\label{lem:quantitative-seam}
Consider the unperturbed mixed equation in the product geometry of
\cite[(1.1)]{DFL-mixed}, with fixed surface metric and its induced
complex structure. Let $Q_2$ be a gauge half-disk times the matching
surface, together with the section half-disk, and let $Q_1$ be the
concentric patch of half the radius. There are $e_*>0$ and $C_k$
such that every solution of energy $e\le e_*$ on $Q_2$ satisfies
\[
 \sup_{Q_1}(|\nabla_A^kF_A|+|\nabla^kdu|)\le C_k\sqrt e.
\]
The constants are uniform on compact families of these product data.
\end{lemma}
\begin{proof}
Choose nested intermediate patches. Mixed small-energy regularity
and compactness give gauges in which solutions of energy below a
fixed concentration threshold have bounded smooth coefficients on
these patches \cite[Theorem~4.2, Lemma~4.25 and pp.~38--39]{DFL-mixed}.
As the energy tends to zero their limits are flat connections and
constant sections. The possible flat surface classes form a compact
space, so finitely many gauge and target charts suffice. These are
qualitative coefficient bounds; the energy rate is obtained next.

Let $t$ be the coordinate tangent to the seam and set
$\zeta=\iota_{\partial_t}F_A$, $\xi=\partial_tu$.
Covariant differentiation of the unperturbed mixed equation in
this product coordinate shows that $(\zeta,\xi)$ satisfies its
linearized equation. The gauge row is zero by the Yang--Mills
identity $d_A^*F_A=0$. Its normal component on the seam is zero,
since anti-self-duality identifies $F_{ts}$ with the curvature of
the flat surface restriction. Differentiating that restriction and
its class gives the closed-trace and harmonic-matching conditions.
Thus $(\zeta,\xi)$ lies in the local mixed boundary domain,
including the gauge row, as in the translation-field argument of
\cite[Proposition~5.29]{DFL}. Use matched scalar cutoffs on the
gauge and section patches, with identical seam values and supports
away from the artificial edges. Their commutators are zeroth order.
The local all-order boundary estimates of
\cite[Theorems~5 and 5.9, Remarks~5.73 and 5.76]{DFL-mixed},
with the uniformity of \cite[Remark~4.19]{DFL}, give
\[
 \|\zeta\|_{H^{j+1}(Q_1)}+\|\xi\|_{H^{j+1}(Q_1)}
 \le C_j\bigl(\|\zeta\|_{L^2(Q')}+\|\xi\|_{L^2(Q')}\bigr)
 \le C'_j\sqrt e,
\]
where $Q_1\Subset Q'\Subset Q_2$. One can equivalently trivialize the moving matching domain by
its smooth chart $Q$ and commute $Q\partial_tQ^{-1}$, rather than
raw tangential differentiation, before recovering normal derivatives
from the equation. Use successively smaller patches. The
qualitative smooth bounds
control every coefficient and its commutators at each fixed order.
They also give uniform constants over the finite chart cover.

For an anti-self-dual two-form on this product, contraction with
$\partial_t$ determines all its components and
$|F_A|^2=2|\zeta|^2$. The Cauchy--Riemann equation similarly
determines $du$ from $\xi$. These identities and Sobolev embedding
prove the claimed bounds. The threshold is chosen once below
concentration; only $C_k$ depends on the derivative order.
\end{proof}

Away from the seam we use the corresponding interior estimates
\cite[Lemma~4.50]{DFL-mixed}.
...
invariant. So Lemma~\ref{lem:quantitative-seam} in that fixed product
geometry and the interior estimate \cite[Lemma~4.50]{DFL-mixed},
which allows a $C^k$-compact family of metrics, apply on every unit patch with constants independent of $\eps$
...
integrating over $t$,
\[
\mathcal J+\|\xi\|^2\le C_P\left(\|\partial_r\xi\|^2+\int\|\mathcal B_tU\|^2dt\right),
\]
where $C_P$ is fixed by the transverse limiting pair. It need not be small.
Return to the energy identities \eqref{eq:gauge-identity} and
\eqref{eq:strip-identity}, before replacing the boundary terms by an
unweighted $L^2$ remainder. Put
\[
\mathcal E_0=\|\partial_t\xi\|^2+\|\partial_r\xi\|^2+
\int\bigl({\mathfrak s}\|\mathsf D_tU\|^2+
{\mathfrak s}^{-1}\|\mathcal B_tU\|^2\bigr).
\]
The coefficients involving $\dot q$, the derivatives of the strip, and
the moving Lagrangian frames tend to zero on the ends. Their contributions,
including the lower-order term of the strip operator, are at most
$o_{T_0}(1)(\mathcal E_0+\mathcal J+\|\xi\|^2)$ after Young's inequality.
The bounded scale derivative contributes at most a fixed multiple of
$H^2\int{\mathfrak s}\|U\|^2$, after absorbing one quarter of the
$\mathcal B_tU$ term. Lemma~\ref{lem:tie} and the preceding Poincar\'e bound give
\[
\int{\mathfrak s}\|U\|^2+
\mathcal J+\|\xi\|^2\le C(1+C_P)\mathcal E_0,
\qquad
\int{\mathfrak s}\|U\|^2\le C{\mathfrak s}_1(1+C_P)\mathcal E_0.
\]
Consequently
\[
\mathcal E_0\le C\|D_{\mathfrak s}z\|_Y^2+
\bigl(o_{T_0}(1)(1+C_P)+C H^2{\mathfrak s}_1(1+C_P)\bigr)\mathcal E_0.
\]
Choose $T_0$ first and then ${\mathfrak s}_1$ to absorb the last term.
The complementary spatial estimate and the same Poincar\'e bound recover
the full $X$ norm from $\mathcal E_0$. For the adjoint the extra scalar
endomorphism is bounded by $CH$ and is treated by the identical small-scale
estimate. This proves both assertions without a small Poincar\'e constant.
\end{proof}
...
Write $H^j_{\rm uloc}(\widehat g_\eps)$ for the supremum of the
$H^j$ norms over a uniform cover by unit patches in the rescaled
metric, with covariant derivatives taken using the reference
connection.
...
\emph{Strong norms.} On the small compression, with $\mathsf w$ the fixed end weight
(equal to one on the region $N\cap X_\eps$ where $X_\eps$ differs from $X_0$, Definition~\ref{def:insertion}(iv)), put
\[
\|f\|_{Y^m_\eps}^2=\sum_{i+r\le m}\|\nabla^i_t(\mathsf w{\mathfrak s}^{1/2}f)\|^2_{L^2_tH^r(C)},
\]
\[
\|z\|_{X^m_\eps}^2=\|\mathsf wv\|^2_{H^m_t}+\sum_{i=0}^m\|\mathsf w{\mathfrak s}^{-1/2}\nabla^i_tU_\perp\|^2_{L^2_t
H^{m+1-i}(C)}+\|\mathsf w{\mathfrak s}^{1/2}\nabla_t^{m+1}U\|^2_{L^2_tL^2(C)},
\]
...
$\mathcal V^j$ gives
\[
D_\eps(\mathcal V^jz)=\mathcal V^jD_\eps z+
\sum_{i<j}\sum_{k=0}^{j-i}{\mathfrak s}^{-1}a_{ijk}(t)
\mathcal B^{[k]}_t\mathcal V^iU+\sum_{i<j}b_{ij}\mathcal V^iz,
\]
where $\mathcal B^{[0]}_t=\mathcal B_t$, all coefficients are bounded,
and $b_{ij}$ collects the terms of order zero. Differentiating the scale
produces the terms with $k=0$, even when the background is constant. By
Lemma~\ref{lem:transport}, for every $k\ge0$ the operator
$\mathcal B^{[k]}_t$ annihilates the harmonic fields and
$\|\mathcal B^{[k]}_tU\|\le C\|\mathcal B_tU\|$, so no factor ${\mathfrak s}^{-1}$ multiplies a slow
term and
\[
\|{\mathfrak s}^{-1}a_{ijk}\mathcal B^{[k]}_t\mathcal V^iU\|_{Y_\eps}\le C\|{\mathfrak s}^{-1/2}\mathcal B_t\mathcal V^iU\|\le
...
$\alpha_{t,v,0}$. At $w=0$ the derivative of the left side in the class equation is
$de_t(v)(\tau_t\dot w+\dot\vartheta)$, whereas that of the right side
is $de_t(v)\tau_t\dot w$. Thus $\vartheta_t(v,0)=0$ and
$D_w\vartheta_t(v,0)=0$ for every $v$.

\emph{Extension in the normal direction.} The harmonic parameter is a
function of the boundary trace of $w$. Its extension must respect
both the time and the normal derivatives in $X^m_\eps$.
Choose a nonnegative $\rho\in C^\infty_c((-1,1))$ with integral one.
At a fixed scale $s\in(0,1]$, in a smooth frame of $T_{q(t)}\Msf$, set
\[
(\mathcal E_s\vartheta)(t,r)
 =\int\rho(u)\vartheta(t-sru)\,du,\qquad r\ge0.
\]
Its boundary value is $\vartheta(t)$. For a varying scale, choose
unit time intervals, a subordinate partition $\sum\psi_j=1$, and
larger intervals on which ${\mathfrak s}/{\mathfrak s}(j)$ and its
logarithmic derivatives are bounded. Extend $\psi_j\vartheta$ by
$\mathcal E_{{\mathfrak s}(j)}$ and sum, with a time cutoff equal to
one on the slightly enlarged support of each summand. The collar
radius is fixed small enough that all translates used in this formula
remain in the larger interval. Denote the result by
$\mathcal E_{\mathfrak s}\vartheta$. The overlap and cutoff constants
are independent of the minimum scale and of the number of intervals.
Define the exact correction by
\begin{equation}\label{eq:trace-extension-chart}
\Theta(t,r)=(\mathcal E_{\mathfrak s}\vartheta(v,w))(t,r),\qquad
\mathcal R(v,w)(t,r)=\chi(r)\mathcal S_{\alpha_{t,v,r}}
                  (\phi_{t,r},\Theta(t,r)).
\end{equation}
At $r=0$ this is the solution of \eqref{eq:class-equation}. Thus the
trace is flat with the prescribed class. The correction and its
first derivative in $w$ vanish at $w=0$, so \eqref{eq:exact-chart}
and the linearized operator are unchanged.

Here is the estimate for this extension. On a constant-scale patch,
write $\mathcal X_s^m$ for the complementary part of $X^m_\eps$,
with end weight one. The trace norm $\mathcal T_s^m$ is given, in
local time Fourier coordinates and surface spectral coordinates, by
\[
\|f\|_{\mathcal T_s^m}^2=
\int\sum_\lambda s^{-1}(1+\xi^2+\lambda^2)^m
 (1+\lambda^2+s^2\xi^2)^{1/2}|\widehat f(\xi,\lambda)|^2\,d\xi.
\]
Equivalent norms are obtained with a finite surface atlas and smooth
cutoffs. For a finite-dimensional harmonic coefficient omit $\lambda$.
The trace and extension estimates are
\begin{equation}\label{eq:anisotropic-trace}
\|w|_{\partial C}\|_{\mathcal T_s^m}\le C_m\|w\|_{\mathcal X_s^m},
\qquad
\|\chi\mathcal E_s\vartheta\|_{\mathcal X_s^m}
 \le C_m\|\vartheta\|_{\mathcal T_s^m}.
\end{equation}
For the first, apply the normal one-dimensional trace inequality
with parameter $(1+\lambda^2+s^2\xi^2)^{1/2}$ to the time and
surface transform. Tangential differentiation gives the factor
$(1+\xi^2+\lambda^2)^m$. The resulting bulk norm is equivalent to
the displayed complementary norm in $X^m_\eps$: terms containing
$s^2\xi^2$ and a spatial derivative are dominated by its spatial
sum, and the remaining highest time term is
$s\|\partial_t^{m+1}w\|^2$. For the second estimate the multiplier
is $\widehat\rho(sr\xi)$. Its derivatives decrease rapidly for
$sr|\xi|\gg1$. Integrating each normal derivative in $r$ gives
\[
\int_0^{r_1}|\partial_r^j(\chi(r)\widehat\rho(sr\xi))|^2\,dr
 \le C_j(1+s|\xi|)^{2j-1},
\]
with the same assertion for $j=0$. Since $s\le1$, inserting the
time derivatives and summing proves \eqref{eq:anisotropic-trace}.
In particular the top time derivative is estimated through a
half-order boundary norm, not an $L^2$ trace of that derivative.

\begin{lemma}[quadratic estimates in the matching chart]\label{lem:block}
Fix $m\ge4$ and a compact smooth family of reference paths $q$.
In the chart \eqref{eq:trace-extension-chart}, write
$\mathcal F_\eps(W_\eps+z)=f_\eps+D_\eps z+N_\eps(z)$.
For a sufficiently small ball, with constants independent of $\eps$,
\begin{equation}\label{eq:strong-quadratic}
\|N_\eps(z)-N_\eps(z')\|_{Y^m_\eps}
 \le C_m(\|z\|_{X^m_\eps}+\|z'\|_{X^m_\eps})
             \|z-z'\|_{X^m_\eps}.
\end{equation}
Let $\mathsf d(z)$ be the supremum of $\|v\|_{C^0}+
\|U_\perp\|_{C^0}$ on compression patches, of $\|a\|_{C^0}$ on
other gauge patches, and of $\|\nu\|_{C^0}$ on section patches.
On a small smooth coefficient neighbourhood one also has
\begin{equation}\label{eq:low-quadratic}
\|N_\eps(z)\|_{Y_\eps}\le C\mathsf d(z)\|z\|_{X_\eps}.
\end{equation}
If $z_*$ is in the strong ball and $z$ is smooth with small
coefficient size, then
\begin{equation}\label{eq:asymmetric-quadratic}
\|N_\eps(z)-N_\eps(z_*)\|_{Y_\eps}
 \le C(\mathsf d(z)+\|z_*\|_{X^m_\eps})\|z-z_*\|_{X_\eps}.
\end{equation}
The same local assertions hold on the component and exterior domains.
\end{lemma}

\begin{proof}
We first describe the surface estimate that is needed in addition to
\eqref{eq:anisotropic-trace}. Freeze $t,v$ and use the harmonic frame
$\mathfrak i_{t,v,0}$. Thus the prescribed linear class is
$h=\mathfrak i_{t,v,0}^{-1}H_\alpha\phi=\tau_tw$, and $A(v,h)$
is the flat slice representative of $e_t(v+h)$ relative to
$\alpha=B_t(v)|_\Sigma$. We write $A(h)$ while $v$ is fixed.
For differentiation, use the following gauge equation also for
nonclosed $\phi$, with $h=\mathfrak i_{t,v,0}^{-1}H_\alpha\phi$.
It defines an extension of the harmonic class parameter off the
closed-form subspace; on that subspace it equals the parameter in
\eqref{eq:class-equation}. The class equation is
implemented by solving
\[
d_\alpha^*\bigl(g^*A(h)-\alpha-\phi\bigr)=0,
\qquad g=\exp\eta.
\]
For a fixed $p>2$ this is an equation from $W^{1,p}$ to
$W^{-1,p}$, with invertible derivative $d_\alpha^*d_\alpha$ at
zero. Small $C^0$ norm of $\phi$ gives a small $W^{1,p}$ solution
$\eta$; hence $\eta$ is small in $C^0$. The differentiated equation,
viewed from $H^1$ to $H^{-1}$, gives
\[
\|D\eta[\dot\phi]\|_{H^1}\le C\|\dot\phi\|_{L^2}.
\]
Indeed its leading coefficient is close in $C^0$ to that of the
covariant Laplacian, and its remaining coefficients are small in
$L^p$. H\"older's inequality and the surface Sobolev inequality
absorb those terms using the irreducible scalar gap.

Now $\vartheta=H_\alpha(g^*A(h)-\alpha-\phi)$ in the chosen
harmonic frame. Harmonic projection kills the exact part of
$D\eta$, and the linear terms $\dot h-H_\alpha\dot\phi$ cancel.
The remaining terms contain $\phi$, $g-1$, or the nonlinear part
of the flat slice. Pairing with smooth harmonic forms and using
the preceding estimate gives
\begin{equation}\label{eq:class-map-L2}
|D_\phi\vartheta[\dot\phi]|
 \le C\|\phi\|_{C^0}\|\dot\phi\|_{L^2}.
\end{equation}
For example, the commutator term is bounded by
$C\|g^*A(h)-\alpha\|_{L^2}\|D\eta[\dot\phi]\|_{L^2}$.
The quantitative differentiated estimates, in the fixed frame, are
\begin{align}
|D_\phi^2\vartheta[\psi_1,\psi_2]|&\le
 C\|\psi_1\|_2\|\psi_2\|_2,\nonumber\\
|D_vD_\phi\vartheta[b,\psi]|&\le
 C|b|\|\phi\|_2\|\psi\|_2,\nonumber\\
|D_v\vartheta[b]|&\le C|b|\|\phi\|_2^2.
\label{eq:class-map-mixed}
\end{align}
To see these bounds, differentiate the weak gauge equation, retaining
it in divergence form. Each differentiated gauge term has at most
one derivative of a gauge parameter. A first variation lies in
$H^1$; products of an undifferentiated $H^1$ variation and an
$L^2$ differentiated variation lie in $L^r$ for every $r<2$.
The inverse weak Laplacian gives $W^{1,r}$ for the next gauge
variation. Surface Sobolev embedding and H\"older's inequality
then bound its harmonic pairings in $L^1$. Repeating this argument
at any fixed finite order gives bounded multilinear harmonic
pairings, choosing the intermediate exponents sufficiently close
to two. No derivative of $\phi$ is used in these estimates.
Differentiating the smooth finite-dimensional flat slice and
harmonic projection adds only terms with the same bounds. The
identities $\vartheta(v,0)=D_\phi\vartheta(v,0)=0$ eliminate the
constant and linear terms in every $v$-derivative. Integrating the
bounded $D_vD_\phi^2\vartheta$ gives the last two estimates in
\eqref{eq:class-map-mixed}, and integrating these differential
bounds gives their difference versions.

Here is an explicit strong trace bound. On a constant-scale patch
put $Z=\|z\|_{X_s^m}$. The trace theorem and the linear matching
identity give
\[
\|\phi\|_{H^m_tL^2_\Sigma}\le C\sqrt{s}\,Z,\qquad
\|\phi\|_{H^{m+1/2}_tL^2_\Sigma}
 +\|v\|_{H^{m+1/2}_t}\le CZ.
\]
For the second bound on $v$, use
$v=\Pi\nu|_\Sigma-\Pi\tau_tw$: the section has $H^{m+1}$
regularity and its trace has $H^{m+1/2}$ regularity, while the
harmonic part of the complementary trace is bounded by
\eqref{eq:anisotropic-trace}. The transport $T_{t,v}$ obeys the
same estimates by the product rule. The one-dimensional tame
product inequalities, applied to the differentiated harmonic
pairings above, now give
\begin{align*}
\|\vartheta\|_{H^m_t}&\le C\|\phi\|_{H^m_tL^2_\Sigma}^2
 (1+\|v\|_{H^m_t}),\\
\|\vartheta\|_{H^{m+1/2}_t}&\le
 C\|\phi\|_{H^m_tL^2_\Sigma}
 \bigl(\|\phi\|_{H^{m+1/2}_tL^2_\Sigma}
       +\|\phi\|_{H^m_tL^2_\Sigma}\|v\|_{H^{m+1/2}_t}\bigr).
\end{align*}
Since $\|\vartheta\|_{\mathcal T_s^m}$ is equivalent to
$s^{-1/2}\|\vartheta\|_{H^m_t}+\|\vartheta\|_{\dot H^{m+1/2}_t}$,
these prove $\|\vartheta\|_{\mathcal T_s^m}\le C\sqrt{s}\,Z^2$
in a fixed small ball. The differentiated inequalities give
\[
\|\vartheta(z)-\vartheta(z')\|_{\mathcal T_s^m}
 \le C\sqrt{s}(\|z\|_{X_s^m}+\|z'\|_{X_s^m})
                  \|z-z'\|_{X_s^m}.
\]
These are the weighted class-parameter estimates used below.

The spatial part of $\mathcal S_\alpha$ is controlled by its
formula $\mathfrak i\Theta+\mathsf K_\alpha(\phi+\zeta,\phi+\zeta)$.
The operator $\mathsf K_\alpha$ gains one surface derivative.
Time and normal derivatives commute through it with bounded
lower-order commutators; the coefficients depend smoothly on the
fixed family and on $v$. For $m\ge4$, the bulk $H^m$ algebra
inequality and the boundary $H^{m-1/2}$ algebra inequality control
all products below the highest order. At the highest time order,
use \eqref{eq:class-map-L2}, its differentiated equation, and
\eqref{eq:anisotropic-trace}. The half-order time seminorm is
estimated by differences before differentiating the extension.
This gives the tame quadratic bound for $\mathcal R$ in
$\mathcal X_s^m$. The same argument applies to the difference of
two corrections. No extra top-order boundary trace is taken.

At first order this argument uses only small coefficients. For
clarity, the harmonic trace norm with $m=0$ is equivalent to
$s^{-1}\|\vartheta\|_2^2+\|\vartheta\|_{\dot H^{1/2}_t}^2$.
Apply \eqref{eq:class-map-L2} to the differences in the second
term. Terms involving a difference of $v$ are treated by splitting
the difference integral at $|t-u|=s$. On the near part,
$|v(t)-v(u)|^2\le |t-u|\int_u^t|v'|^2$ supplies the weighted
quantity $s\|v'\|_2^2$; on the far part use $2\|v\|_\infty$
and the $s^{-1}\|\phi\|_2^2$ term. The two factors of $\phi$ in
a $v$-derivative ensure that the undifferentiated factor is a
small coefficient. This proves the absolute first-order estimate
for the boundary correction. Expanding about a strong reference
puts its differentiated coefficients on that reference and treats
the unknown remainder quadratically; it gives the asymmetric
version by the same difference-integral argument. More explicitly,
write $y=z-z_*$ and use the integral Taylor formula at $z_*$.
Terms quadratic in its complementary remainder are bounded by
$C\mathsf d(y)\|y\|_{X_s}$. In the linear and mixed terms,
$\|\phi_*\|_{C^1_tC^0_\Sigma}\le C\sqrt{s}\,a$ and
$\|v_*\|_{C^1}\le Ca$, where $a=\|z_*\|_{X_s^m}$.
Multiplication by these coefficients is bounded on the half-order
time norm. A term linear in the slow remainder has coefficient
$O(s a^2)$ by \eqref{eq:class-map-mixed}; this cancels the
$s^{-1/2}$ in the low-frequency trace norm, and the high-frequency
part is controlled by the matching bound
$\|v_y\|_{H^{1/2}}\le C\|y\|_{X_s}$.
The near/far split treats the remaining mixed differences.
Consequently
\[
\|\vartheta(z)\|_{\mathcal T_s^0}\le C\mathsf d(z)\|z\|_{X_s},
\quad
\|\vartheta(z)-\vartheta(z_*)\|_{\mathcal T_s^0}
 \le C(\mathsf d(z)+a)\|z-z_*\|_{X_s}.
\]

The other chart terms have the same estimates directly. In the
surface Hodge transport, the principal symbols do not change with
the reference flat connection. Thus $T_{t,v}-1$ has order at most
$-1$ on the fixed surface and is bounded on $C^0$, with norm
$O(|v|)$; its finite-dimensional harmonic part has the same bound.
The spatial operator vanishes on the exact flat family $B_t(v)$.
Every singular remaining term has a complementary factor, measured
with $s^{-1/2}$ in $X_s$. A differentiated slow factor in the
temporal terms carries $s^{1/2}$: in particular
$s^{1/2}|v'w|$ is bounded by $\|w\|_\infty$ times the weighted
temporal norm of $v$. The terms from $T_{t,v}-1$, the scalar row,
and the local bracket are bounded in this way. The pure section
operator has the usual quadratic estimate in its fixed chart.
These estimates prove all three inequalities at constant scale.
Finally freeze the weights on the unit patches used above and sum.
Bounded logarithmic derivatives, bounded overlap and the support
of $\rho$ give the same constants for the varying-scale extension.
\end{proof}

The chart is locally invertible also in uniformly local rescaled
H\"older norms: use $T=t/s$ on a constant-scale patch. The extension
is then $\int\rho(u)\vartheta(T-ru)du$, bounded in $C^\alpha$;
the surface constructions and harmonic projections are smooth in
these norms, and the correction has zero derivative at zero.
The inverse function theorem gives a uniform small neighbourhood.
Smooth fields with small $C^0$ coefficients and uniformly bounded
higher rescaled derivatives enter it by interpolation. This is the
neighbourhood used for the unknown solutions in the proof of surjectivity.
First differentiate the inverse equation in time and in the surface
directions. The leading operator remains a small perturbation of
the identity on the local H\"older space. Then recover normal
regularity from the explicit extension: each normal derivative of
$\int\rho(u)\vartheta(T-ru)du$ is an integral of a time derivative
of $\vartheta$, already controlled by the first step. This triangular
induction preserves every fixed higher rescaled local bound; a
normal derivative of the extension is not a lower-order commutator. At a fixed positive $\eps$ the changed region
is compact, so these smooth local bounds imply finite first-order
norm there. On the original ends subtract the free stationary
limits first. The extension has uniformly bounded time support and
is bounded on the fixed weighted spaces; the trace estimates and
the small-coefficient inverse give the same weighted bounds for the
remainders. Thus the inverse preserves the end limits and finite
$X_\eps$ norm; this assertion uses the trace estimates as well as
local H\"older invertibility.
...
Fix $e_0>0$ small. Let $J$ be an interval in the neck and let $J^+$
contain its closed three-unit neighbourhood in the complete neck
coordinate. Include in $J^+$ the full fixed-radius seam half-disks
used below. If the energy $e$ on this larger region is at most $e_0$,
then, for $\nu$ large, the following hold on $J$, uniformly in its
position and in $\nu$:
\begin{enumerate}[label=(\roman*)]
...
correspond to regions of size $C{\mathfrak s}_b$. Choose their inner
patches to cover $J$ and their larger patches inside $J^+$, decreasing
the fixed patch radius if necessary. The energy on each larger patch
...
than with vanishing energy. Apply it to $[s_0,L-s_0]$ using the energy
on $[s_0-3,L-s_0+3]$ and its seam patches, choosing $s_0$ larger first.
It gives a uniform Hölder bound for the
...
\emph{Coordinates.} Write $a_{\rm aff}$ for the affine difference
produced by Lemma~\ref{lem:global-gauge}. Its coefficients tend to
zero, and its higher rescaled local norms are bounded. The rescaled
H\"older inverse of the chart therefore applies. Write $x$ for the
resulting exact matching coordinate; it is smooth, has finite
$X_\eps$ norm, and $\mathsf d(x)\to0$. We need only
\begin{equation}\label{eq:weak-chart-comparison}
\|x\|_{Z_\eps}\le C\|a_{\rm aff}\|_{Z_\eps}
                     +C\mathsf d(x)\|x\|_{X_\eps}.
\end{equation}
Indeed $a_{\rm aff}-x$ is the sum of the slow quadratic term,
the transport correction, and $\mathcal R(v,w)$, with the analogous
section correction. The first two are bounded in $Z_\eps$ by
$C\mathsf d(x)\|x\|_{X_\eps}$. For the third use the trace and
extension estimates \eqref{eq:anisotropic-trace} at order zero and
the surface estimate \eqref{eq:class-map-L2}. Its physical $Z$ norm
has the factor ${\mathfrak s}^{1/2}$, so the bound follows from the
first-order correction estimate. The section correction has the
ordinary $C^0$ quadratic bound. This proves
\eqref{eq:weak-chart-comparison}; it makes no assertion that the
boundary-dependent correction is bounded on bulk $L^2$ alone.
The gauge-fixing row is the affine Coulomb condition composed with
the chart.
...
$C^0$ smallness. Proposition~\ref{prop:attached-garding},
\eqref{eq:weak-chart-comparison}, and \eqref{eq:low-quadratic} give
$\|x\|_{X_\eps}\le C(e_\eps+\|a_{\rm aff}\|_{Z_\eps})+
C\mathsf d(x)\|x\|_{X_\eps}$. Absorbing the last term gives
\[
...
\begin{proof}
First give each mixed end its cylindrical conformal representative.
Conformal invariance preserves the solution space and surjectivity
modulo gauge; the Coulomb row and weighted realization are chosen
for this representative. The space of these domains and data,
with fixed end types, is connected: the
...
Let $B$ be the Euclidean plane, a half-plane, or a straight strip,
with the product metric on $T\times B$. Let
$D=(d_\alpha^*,\sqrt2\,d_\alpha^+)$ act on $\ad$-valued one-forms,
with $a(\nu)=0$ and $d_\alpha(a|_T)=0$ on the boundary. Both $D$
and its adjoint are invertible and
$\|D^{-1}\|_{L^2\to L^2}\le\mu_T^{-1}$, independently of the strip
width. Conjugating by $e^{\delta\tau}$, where $|d\tau|\le1$ and
$\tau$ is constant on torus fibres, gives the bound
$(\mu_T-|\delta|)^{-1}$ for $|\delta|<\mu_T$.

For the boundary-flux statement use oriented coordinates $(s,t,x,y)$,
where $s\ge0$ is the inward normal base coordinate and $(x,y)$ are
oriented orthonormal coordinates on the flat torus. If $A$ is
anti-self-dual and has flat torus restriction at $s=0$, then
\[
\int_T\langle\nabla_sF_A,F_A\rangle
 =4\int_T\langle[F_{sx},F_{sy}],u\rangle,
\qquad d_{A|_T}u=\iota_{\partial_t}F_A|_T .
\]
The scalar $u$ is unique because $H^0(T;\ad\alpha)=0$.
\end{proposition}

\begin{proof}
\emph{The operator and its boundary conditions.}
Complexify the flat adjoint bundle and decompose into its twisted
Fourier modes. There is no zero mode. On a mode of torus frequency
$k\ge\mu_T$, rotate the orthonormal torus frame so that its covector
is $k\,dx$ and write $a=f\,ds+g\,dt+p\,dx+q\,dy$.
In the isometric identification of self-dual forms with three
components, the four components of $Da$ are
\[
-f_s-g_t-ikp,\quad g_s-f_t+ikq,\quad
p_s-ikf-q_t,\quad q_s+p_t-ikg.
\]
At a straight boundary the conditions are $f=q=0$: the latter is
exactly the closedness of the torus restriction for this mode.
Integration by parts cancels all cross terms and gives
\[
\|Da\|^2=\|\partial_sa\|^2+\|\partial_ta\|^2+k^2\|a\|^2.
\]
The boundary Green form pairs the free components $g,p$ with the
second and third output components. Thus those output components
vanish in the adjoint domain. In invariant notation these conditions
are $\omega(\nu,\tau_B)=0$ and
$d_\alpha^*((\iota_\nu\omega)|_T)=0$; the scalar output is free.
The same calculation for the adjoint gives the identical coercive
bound. Summing over the torus modes proves the estimates on the
plane, half-plane and strip. The closed-range theorem and the
vanishing adjoint kernel give surjectivity. The argument uses no
constant depending on the distance between the two strip boundaries.
Conjugation by the weight adds a zeroth-order principal-symbol term
of norm at most $|\delta|$; a Neumann series proves the weighted claim.

\emph{The flux.} Locally along the boundary, choose a gauge with
$A|_T=\alpha$, extend it by $A_s=0$, and set $\Phi=A_t$ there.
The anti-self-duality equations give
\[
F_{st}=F_{xy}=0,\qquad F_{sx}=-D_y\Phi,\qquad
F_{sy}=D_x\Phi,\qquad u=-\Phi.
\]
Differentiate the anti-self-duality identities in the collar, use
Bianchi there, and then restrict to the face. This gives
\[
\int_T\langle\nabla_sF_A,F_A\rangle
=2\int_T\bigl(\langle D_x\Phi,D_tD_y\Phi\rangle
             -\langle D_y\Phi,D_tD_x\Phi\rangle\bigr).
\]
The ordinary $t$-derivatives cancel after torus integration by parts,
since $[D_x,D_y]=0$. The remaining commutators give
$-4\int_T\langle\Phi,[D_x\Phi,D_y\Phi]\rangle$, which is the
asserted expression. All terms are gauge invariant, so the local
calculation suffices along the face. The odd-torus scalar gap bounds
$u$ by $d_{A|_T}u$, explaining the cubic curvature order.
\end{proof}
...
\emph{A fixed Neumann slice.} Choose rounded contractible domains
$\Omega_1\Subset\Omega_2\Subset D^+_2$, with relative closure along
the physical diameter, such that $D^+_1\Subset\Omega_1$.
The qualitative compactness argument of
\cite[Theorem~4.2, Lemma~4.25 and pp.~38--39]{DFL-mixed}, applied
with the one-point torus moduli space, implies that connections
whose energy tends to zero converge smoothly in gauges on
$T\times\Omega_2$ to a flat connection. That connection is gauge
equivalent to $\alpha$: the base is contractible and the odd torus
has a single flat class. On a slightly smaller rounded domain,
still denoted by $\Omega_2$, impose
\[
d_\alpha^*a=0,\qquad a(\nu)=0
       \quad\hbox{on the whole boundary of }T\times\Omega_2.
\]
There is no prescribed gauge on the artificial boundary. The scalar
Neumann linearization is invertible since $H^0(T;\ad\alpha)=0$,
so the slice follows from the implicit function theorem. In this
slice $a$ tends to zero smoothly as the energy tends to zero.
The absolute twisted cohomology in degree one vanishes on
$T\times\Omega_2$. The Hodge estimate and four-dimensional
Sobolev embedding therefore give
\[
\|a\|_{H^1}\le C\|d_\alpha a\|_2
 \le C\bigl(\epsilon+\|a\|_4^2\bigr)
 \le C\epsilon+C'\|a\|_{H^1}^2.
\]
Qualitative smallness absorbs the last term. Thus
$\|a\|_{H^1(T\times\Omega_2)}\le C\epsilon$.

\emph{The boundary estimate.} On the physical face let
$P_{\rm coex}$ denote fibrewise coexact projection and set
$Q_T=d_\alpha^*(d_\alpha d_\alpha^*)^{-1}$ on torus two-forms.
Flatness is equivalent to
\[
P_{\rm coex}a_T=-\tfrac12Q_T[a_T\wedge a_T].
\]
The operator $Q_T$ is bounded on each isotropic Sobolev space
on $T\times\RR_t$; no gain in the $t$ direction is asserted.
For a form $w$ supported in a coordinate half-disk and with zero
normal component, the linear estimate is
\[
\|w\|_{H^{j+1}}\le C_j\bigl(\|Dw\|_{H^j}
 +\|P_{\rm coex}w_T\|_{H^{j+1/2}}+\|w\|_2\bigr).
\]
For zero coexact trace, its $H^1$ version is the Fourier calculation
in Proposition~\ref{prop:flat-fibre}. Tangential derivatives commute
with $D$ and preserve the boundary condition; the equation recovers
normal derivatives. For nonzero trace, subtract its Poisson extension,
which maps $H^{j+1/2}$ to $H^{j+1}$ and has zero normal component.
This proves the displayed estimate at every integer $j\ge0$.

Choose a scalar base cutoff $\chi$ and apply the estimate to
$w=\chi a$. Its equation and boundary value are
\[
Dw=-\sqrt2\chi(a\wedge a)^++[D,\chi]a,
\qquad P_{\rm coex}w_T=-\tfrac12Q_T[a_T\wedge w_T].
\]
At each fixed order, qualitative smooth smallness of $a$ makes
multiplication by $a$, both in the interior and on the boundary,
small in the displayed norms. The trace theorem therefore absorbs
the terms containing the highest derivatives of $w$ into the same
$H^{j+1}$ norm on the left. The cutoff commutator contains only
$j$ derivatives of $a$. Induction on nested half-disks, starting
with the $H^1$ estimate, proves the $H^{j+1}$ bound $C_j\epsilon$.
Interior patches have the same argument without the trace term.
Sobolev embedding then gives the asserted connection and curvature
bounds at each fixed order, initially below an order-dependent
threshold $\hbar_k$.

\emph{A common energy threshold.} Fix $\hbar$ below the concentration
threshold in the cited compactness theorem and small enough to
enter the single low-order Neumann slice on $T\times\Omega_2$.
The family of energy at most $\hbar$ is smoothly precompact modulo
gauge on smaller domains: any failure would give concentration,
whose energy is bounded below by that threshold. The normalized
Neumann representatives are consequently smoothly precompact on
$T\times\Omega_1$; the slice correction is unique near the identity
and elliptic regularity controls it at every higher order. Let
$M_k$ be a bound for their $C^k$ connection and curvature norms.
For energies $E\in[\hbar_k,\hbar]$ use
$M_k\le(M_k/\sqrt{\hbar_k})\sqrt E$, and for $E<\hbar_k$ use the
linear estimate just proved. Increasing $C_k$ gives (b) with one
$\hbar$ for all $k$.
...
solutions on $[\theta-2,\theta+2]\times Y''$, where
$Y''=Y\cup\{r\le4\}$, whose kinetic energy tends to zero converge,
after gauge transformations, in $C^\infty$ on
$[\theta-1,\theta+1]\times Y'$ to a stationary pair. For
$\mathsf R\in[0,4]$ use $\psi_\mathsf R$ and
Theorem~\ref{thm:flat-boundary-regularity} at the physical face;
compactness of this parameter interval makes the bounds uniform.
Part (i) is gauge invariant. On the unperturbed collar interior
regularity and Theorem~\ref{thm:flat-boundary-regularity}(a) give
a smooth representative, with local gauges patched as in Step~1
of Lemma~\ref{lem:adapted}. Apply the torus slice lemma to this
representative. On the collar,
...
satisfies the linearized equation by translation invariance.
In temporal gauge its gauge row vanishes:
$d_A^*\zeta=-d_B^*(*F_B+\nabla h(B))=0$, by Bianchi and gauge
invariance of the cylinder function \cite[Proposition~2.10]{DFL}.
At the seam its normal component is $F_A(\partial_\theta,\partial_s)=0$:
anti-self-duality identifies this component with the curvature on
the flat surface slice. Thus it belongs to the boundary domain
of the gauge-fixed linearization. The local estimates of
\cite[Proposition~5.27 and Remark~4.19]{DFL}, on the larger region
$Y''$, give
$\sup_{[\theta,\theta+1]\times Y'}|\nabla_A^k\zeta|
\le C_k\|\zeta\|_{L^2([\theta-1,\theta+2]\times Y'')}$
when the kinetic energy is below a threshold depending on $k$.
Choose $e'_m\le e_0$ below these
...
root of the local energy, cover each unit square $Q$ by a fixed
finite collection of disks $D_1(z)$ whose larger disks $D_2(z)$
lie in $\{r\ge S'_m\}$. Apply the estimate on these larger disks
for every $i\le m$, taking the minimum of the finitely many energy
thresholds. The weight varies by a bounded factor on each disk.
Summing over a cover with bounded overlap,
\[
...
the last step is carried out level by level on nested locally finite
covers. In each initial ball use radii $\rho_i(1+2^{-k})$ at
level $k$, with the balls of radius $\rho_i$ still covering.
Fix the local Coulomb gauges at level $m$ and shrink one step
for each interior elliptic estimate. If the chart representatives are of class $L^2_k$ and
...
\cite[Proposition~3.1 and Section~3.2]{Kronheimer-higher}.
The elliptic estimate for
...
vanishing near the genus matching lines and, when present, the
flat torus face. Then there is an even gauge transformation of class
...
genus parts in $\{|\operatorname{Re}+3|<1\}\cup
\{|\operatorname{Re}-2\lambda-3|<1\}$, at distance at least two
...
$C^\infty$ on compact subsets of each component of the broken limit.
For every $\varsigma>0$ there are compact sets in all its components
such that, for large $j$, the energy outside their images in the
finite domain is less than $\varsigma$; trajectory components are
translated by their breaking parameters. If no trajectory breaks,
these are fixed compact sets in the completed core domains. In
particular this is the case when $d=0$ under the stated index assumptions.
The same statements hold for sequences of solutions on a fixed
...
and the energy outside the union of compact sets in the core and
translated trajectory components is small when those sets are large.
When there is no breaking, only the core compact sets are required.
For a fixed completed domain the same
...
further invariant perturbation, at this fixed finite $\ell$,
makes the index-one spaces regular. Its output region includes the
compact neck where the fixed term $\chi f_Z$ has output; it remains
away from the matching lines. Choose it small enough that openness
of regularity and compactness preserve the already regular
index-zero count. This finite-length regularization is not required
to have support avoiding that closed neck; the joined count is
...
Theorem~\ref{thm:regularity}, one can choose a common compact
output and dependence region in the original core, away from the
inserted torus region and the matching lines, such that the completed domains carry no solutions of negative index, and all
...
is compact. Apply Theorem~\ref{thm:regularity}(ii) with output region in the
original pieces, including their original ends, and excluding the
inserted torus collar. Every fixed primary or secondary output lies
there. Each side retains an original odd reference end; in the
joined problem use its two separate output regions. Thus the
hypotheses of the output-annihilator argument hold without loops
crossing the bridge. Part (iii) selects finitely many directions and
a generic small choice making index $d$ regular; a negative index
then does not occur. At the subsequent steps remove neighbourhoods
of already regular broken trajectories before taking a finite cover.
Ordinary end gluing remains valid with the completed torus tail:
the cutoff and error are on the original end, while the fixed tail
is controlled by its positive torus gap and the compact-remainder
Fredholm estimate. No invertibility of an unregularized completed
operator is used in this local gluing assertion.
Repeat through index one. The selected loop images and outputs have
compact union. Enlarge the common core and original-end truncations
to contain this union, and fix them before the finite collar or
bridge lengths vary. Beyond those truncations the original
asymptotic equations are unchanged. This proves the lemma.
...
Lemma~\ref{lem:adapted}(i)--(ii), using (i) for the pointwise
$C^m$ bound.
\end{proof}
...
invariant under homotopies of the transformed data with fixed ends
\cite[Theorems~3.9, 3.13, 4.1, 4.2 and Example~2.6]{WW-relative}. Both boundary
Lagrangians may move. We restrict paths and their homotopies to the
small convex neighbourhoods of exact-graph primitives furnished by the
primary balls. Thus the continuations used here depend only on their
endpoints; no assertion about arbitrary Hamiltonian loops is needed.
...
torus genus is at least two throughout. For a fixed splitting,
shrink the admissible collars so that the fixed height solid tori
remain in the compression interiors. Cutting along the framing
sphere $S$ gives neat collars on the two sides, matched along the
collar of $\partial D$. Uniqueness of neat collars up to relative
isotopy, followed by isotopy extension fixing $\Sigma_h$ and
preserving $S$, joins the collar choices away from the fixed torus
collar. Product metrics on a fixed collar form a convex set. The ambient
small primary parameter neighbourhoods are also convex. An edge
\ref{P:model} joins admissible endpoints within these neighbourhoods;
its intermediate primary pairs need not stay transverse. Thus the
choices are joined by edges~\ref{P:model}.
\end{proof}
...
Fix the required smallness of the total addition before the finite induction, choosing each
increment small enough to meet this requirement. Let $G$ be the family of all cylinder
functions added; this family may depend on the prescribed smallness bound.
The set $\mathcal U'(t_*)$ of the paragraph
...
\bibitem{Kronheimer-higher}
P.~B. Kronheimer, \emph{Four-manifold invariants from higher-rank
bundles}, J. Differential Geom. \textbf{70} (2005), 59--112.
