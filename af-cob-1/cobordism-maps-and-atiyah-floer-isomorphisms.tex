...
\hypersetup{pdfauthor={Bernd Johannes Wuebben},
  pdftitle={Cobordism maps and Atiyah-Floer isomorphisms for framed instanton homology},
...
\title[Cobordism maps and Atiyah--Floer isomorphisms]{Cobordism maps
and Atiyah--Floer isomorphisms\\ for framed instanton homology}
...
within that class, a point not addressed in \cite{DFL} (Corollary~\ref{cor:dfl-natural}). DFL
state that such primary data exist after a constant rescaling of the metric
\cite[Proposition~6.8]{DFL}; whether they exist at the fixed metric data of our presentations
is a transversality question that we leave open (Remark~\ref{rem:dfl-regular}).
...
orientations of the gauge side. Neither is addressed here. The objects
...
(Proposition~\ref{prop:dfl-comparison}); the existence of such data at the metric data of a
presentation is left open (Remark~\ref{rem:dfl-regular}). The maps of the transitive system
...
Sections~\ref{sec:two-handles} and~\ref{sec:torus} and enough further
...
Theorem~3.29 there; see Remark~\ref{rem:dfl-regular} on their existence at the metric data
of a presentation. The gauge complex
...
DFL state that DFL-regular data exist after a constant rescaling of the metric of $M^\#$
\cite[Lemma~2.56 and Proposition~6.8]{DFL}. Their inductive step adds cylinder functions
with loops in $\operatorname{int}C_0$ and in $\operatorname{int}C_1$, and uses, for each cokernel
element $\psi$ of the linearization at a trajectory $\gamma$, the independence of $\psi$ and
$\partial_s\gamma$ on one of the two pieces, which is inferred there from their linear
independence on $M^\#\setminus\Sigma$; the rescaling then moves a unit collar off the
supports. It preserves the regularity of the trajectories, which it only reparametrizes in
time, and the Lagrangians, since the two-form $*_3\nabla h$ does not change under a constant
rescaling of the metric, but it changes the flat metric of the torus collar. At the metric
data of a presentation the same step is needed with $C_i^\circ$ in place of
$\operatorname{int}C_i$. Neither version follows from the $L^2$-orthogonality of $\psi$ and
$\partial_s\gamma$ on $M^\#$ alone: $\psi=c_i\,\partial_s\gamma$ modulo gauge on $C_i$ with constants
$c_0\ne c_1$ is compatible with it, and for such $\psi$ every cylinder function with loops in
one piece pairs to zero. The argument of \cite[Section~5.5, Proposition~5.17]{Donaldson-book}
goes through if, at each stage of the induction, every such pair $(\gamma,\psi)$ admits a time
near which $\psi$ is not a constant multiple of $\partial_s\gamma$ modulo gauge on $C_0^\circ$ or on
$C_1^\circ$ and the restricted path is injective and irreducible. We do not settle this here,
and the results below that concern DFL-regular data are stated for data that have them.
\end{remark}
...
\begin{enumerate}[label=(\roman*)]
\item For $f$ sufficiently small and generic invariant secondary
  perturbations $\eta$, every $\cM^d(a,x)$ with $d\le1$ is regular, the
...
\begin{proof}
\emph{Energy and index.} For a solution of~\eqref{eq:mixed} in the
homotopy class of $(A,u)$, the analytic energy equals the topological
energy plus $2\|\eta(A)\|^2$ plus the term
$\int\chi'(s)f(A_s)\,ds$, whose absolute value is at most $\sup|f|$,
since $\int|\chi'|=1$ and $f$ vanishes at the critical point at the
gauge end. The index is
$\ind D_{(A,u)}=8\kappa(A,u)+\mathfrak c(x)-\mathfrak c(a)$, where
$\kappa$ is the topological energy normalized as in
\cite[Proposition~3.20]{DFL}; the \emph{asymptotic labels} of a configuration are the
...
\emph{Triangularity.} If $\sup|f|+\sup\|\eta\|^2$ is less than the
smallest positive topological energy that can occur, an index-zero
solution has $\kappa\ge0$, so $\mathfrak c(a)\ge\mathfrak c(x)$, and
equality forces $\kappa=0$. The index-zero solutions with $\kappa=0$
converge, as $(f,\eta)\to0$, to the constant pairs of
\cite[Section~3.2]{DFL}, which are regular by
\cite[Proposition~3.19]{DFL}; by the implicit function theorem there is
exactly one near each constant pair. This is the argument of
\cite[Theorem~3.29]{DFL}.
...
If for an admissible presentation $p$ with primary data $\mathfrak h$ and some finite family $G$
there are DFL-regular primary data $\mathfrak h+g$ with $g\in\operatorname{span}G$ arbitrarily small
(Remark~\ref{rem:dfl-regular}), then those with $g$ small enough are admissible, by the
monotonicity of the admissible neighbourhoods (the paragraph after
Proposition~\ref{prop:duality}), and Proposition~\ref{prop:dfl-comparison} applies there.
...
\ref{H:lattice}--\ref{H:framing}. There is $R_0$ such that for
$\min_eR_e\ge R_0$:
\begin{enumerate}[label=(\roman*)]
...
\item the joined count is the composite of the component counts over
  $\FF$;
\item the same holds with one component an index-one trajectory, modulo translation, and with one component an index $-1$ piece of a
...
we describe its four steps and the one point that differs for the mixed
equation.
...
operator of the joined problem at the preglued configuration has a right
inverse bounded independently of the neck lengths, by the patching argument of
the proof of \cite[Proposition~3.9]{Donaldson-book}.

\emph{Existence and local uniqueness.} The preglued configuration has error
decaying exponentially in $\min_eR_e$, because the component solutions converge
exponentially to nondegenerate limits. The quadratic estimate holds with
uniform constants: on closed necks this is \cite[Theorem~4.17]{Donaldson-book},
and on ordinary necks, whose geometry is translation invariant, it follows from
the local estimates of \cite[Section~4]{DFL-mixed} on unit patches. The contraction
mapping principle gives a unique solution near the preglued configuration.
This is the only point where the mixed equation enters, and monotonicity is
used only through compactness.

\emph{Surjectivity.} By compactness and energy exhaustion, every sequence of
solutions with $\min_eR_e\to\infty$ is eventually exponentially close to a
preglued configuration on the necks, uniformly because the cross-sections are
invertible, and hence lies in the neighbourhood where the contraction gives
uniqueness \cite[Section~5.3]{Donaldson-book}. Equivariance and the
...
invariant data. The translation-invariant data on the mixed ends may differ
...
kernel, so Lemma~\ref{lem:family-stationary} gives the chain homotopy.
\end{proof}
...
There are $e_0,c,C_m$, independent of $\mathsf R\in[0,\infty]$ and of $\lambda\ge\lambda_0$,
such that a solution of the autonomous mixed equation on a half-cylinder over
$Y_\mathsf R$ (or $D_{i,\lambda}$) with kinetic energy less than $e_0$ converges to a
stationary solution $(B_q,q)$ in the sense of~\eqref{eq:end-decay} below: its kinetic
energy beyond $\theta$ decays at rate $2c$, and, in a temporal gauge, its difference
from the stationary limit, section included, has $H^m$ norm on unit intervals
$[\theta,\theta+1]$ of the whole slice decaying at rate $c$.
\end{proposition}

\begin{proof}
\emph{Convergence into a neighbourhood of the critical family.} On each unit
interval $[\theta,\theta+1]$ the kinetic energy is less than $e_0$. On the compact part of the
slice local compactness applies, and on the collar
Theorem~\ref{thm:flat-boundary-regularity}(b), summed over a cover of $T\times[0,\mathsf R]$ by
half-disks and disks with bounded overlap, bounds the curvature linearly by the
square root of the local energy. Hence for $e_0$ below a threshold independent of
$\mathsf R$ every slice lies in a fixed neighbourhood of the critical family.

\emph{Gradient inequality.} On this neighbourhood the local action $\mathcal F$ is well
defined, with critical set the critical family, which is Morse--Bott with the
uniform gap $\gamma$ of Proposition~\ref{prop:stationary-gap}. Expanding $\mathcal F$ about
the nearest critical point, in the slice orthogonal to the kernel, gives
$|\mathcal F-\mathcal F_{\rm crit}|\le C\|\nabla\mathcal F\|^2$ with $C$ depending only on $\gamma$; the
flat-fibre condition removes the boundary term at $r=\mathsf R$ in the Hessian, so $C$ is
independent of $\mathsf R$.

\emph{Decay.} Along a solution $\partial_\theta\mathcal F=-\|\nabla\mathcal F\|^2\le-(\mathcal F-\mathcal F_{\rm
crit})/C$, so the kinetic energy $\mathcal F(\theta)-\mathcal F_{\rm crit}$ beyond $\theta$ decays at
the rate $2c=1/C$, and the length of the path of slices over $[\theta,\infty)$ is at most
the sum over unit intervals of the square roots of their kinetic energies, which
decays at the rate $c$; so the solution converges to a stationary solution $(B_q,q)$.
In a temporal gauge the difference from $\pi^*B_q$ solves the linearized equation with
coefficients bounded by the same quantity, and local elliptic estimates on
$[\theta-1,\theta+2]$ times the slice, uniform in $\mathsf R$ by
Theorem~\ref{thm:flat-boundary-regularity} on the collar, give the $H^m$ bound on
$[\theta,\theta+1]$ at the rate $c$.
\end{proof}
...
the Klein four-group, whose three nontrivial elements are odd. We use
Proposition~\ref{prop:growing-ends} in the following form: if $E(\theta_0)<e_0$ is
the kinetic energy of a solution beyond $\theta_0$ on a half-cylinder over $Y_\mathsf R$
(or $D_{i,\lambda}$), then in a temporal gauge, for $\theta\ge\theta_0+1$,
\begin{equation}\label{eq:end-decay}
E(\theta)\le E(\theta_0)e^{-2c(\theta-\theta_0)},\qquad
\|U-\pi^*B\|_{H^m([\theta,\theta+1]\times Y_\mathsf R)}\le C_mE(\theta_0)^{1/2}e^{-c(\theta-\theta_0)},
\end{equation}
where $\pi^*B$ is the stationary limit and the section is included in the norm.
...
Let $\gamma_T=\mu_T/4$, fix $m$, and let $0\le\delta\le\delta_T$, where
\[
\delta_T=\min\{\mu_T/4,\ \mu_T^2/8,\ c/2\},
\]
$c$ is the rate in~\eqref{eq:end-decay}, and the weight on the torus region is
$\tau_0(\theta)$ with $|\tau_0'|\le1$ and $|\tau_0''|\le1$. There are $S_0\ge3$ and
$C_m$ such that:
\begin{enumerate}[label=(\roman*)]
\item $\sup_{T\times D_1(z)}|\nabla_A^kF_A|\le C_ke^{-\gamma_T(r(z)-S_0)}$ for every $z$
  with $r(z)\ge S_0+2$, uniformly in $\theta(z)$;
\item Lemma~\ref{lem:torus-slice} applies on $\Omega=(S_0,\infty)\times\RR_\theta$,
  and in the torus slice $A=\alpha+a_V$ with
  \[
  \|a_V\|_{H^m_\delta(T\times[S,\infty)\times\RR)}\le C_me^{-\gamma_T(S-S_0)}\qquad(S\ge S_0+2),
  \]
...
$\sup_{r\ge S_0-1}|\nabla_A^kF_A|\le e_T$ for $k\le m$, together with local gauges
in which $\|A-\alpha\|_{C^1}\le e_T$. So Lemma~\ref{lem:torus-slice} applies on
$(S_0-1,\infty)\times\RR$, and on each fibre $A=\alpha+b$ with $\|b\|_{C^0}\le Ce_T$.
...
\emph{Integrability in $\theta$.} On each mixed end the slice is $Y_\infty$, and by
Proposition~\ref{prop:growing-ends} and~\eqref{eq:end-decay} the curvature of $V$ on
$[\theta,\theta+1]\times Y_\infty$ has $H^2$ norm at most $Ce^{-c|\theta|}$; by the trace
and Sobolev inequalities on unit boxes, $e+|\partial_\theta e|\le Ce^{-2c|\theta|}$
uniformly in $r$ for $|\theta|$ large. Since $2\delta\le c<2c$, the function
$f(r)=\int_\RR e^{2\delta\tau_0(\theta)}e(r,\theta)\,d\theta$ is finite and of class
$C^2$ for $r\ge S_0-1$, the integrations by parts below have no boundary terms,
and $\int_{S_0}^\infty f(r)\,dr<\infty$.
...
\emph{Higher norms.} By small-energy regularity, which is linear in the square root of
the local energy, $\|\nabla_A^iF_A\|_{L^2(T\times Q)}\le C_i\|F_A\|_{L^2(T\times2Q)}$ for every
unit square $Q$ with $r\ge S_0+1$ on $Q$; the weight varies by at most the factor
...
\le C\int_{S-2}^\infty f(r)\,dr\le Cf(S_0)e^{-\gamma_T'(S-2-S_0)} .
\]
Integrating~\eqref{eq:torus-slice} with $k=m-1$ against the weight gives (ii),
since $\gamma_T'/2>\gamma_T$. For (i), small-energy regularity on $T\times D_2(z)$ bounds
$\sup_{T\times D_1(z)}|\nabla^k_AF_A|$ by $C_k(\int_{r(z)-2}^{r(z)+2}f)^{1/2}$, because
$e^{2\delta\tau_0}\ge1$.
\end{proof}
...
ends $e$. There are $S_V\ge S_0+3$, $\Theta_V$ and a representative $V^{\rm ad}$ of
$V$, obtained from $V$ by an even gauge transformation, such that:
\begin{enumerate}[label=(\roman*)]
\item on $T\times[S_V,\infty)\times\RR_\theta$, $V^{\rm ad}=\alpha+a_V$ is the torus slice, so
  Proposition~\ref{prop:tail-decay}(ii) applies to $a_V$;
\item on each mixed end $e$, with outward end coordinate $\theta$,
  ...
  as $\Theta\to\infty$;
\item on the original gauge and ordinary ends $V^{\rm ad}$ has the standard
  exponential asymptotics at nondegenerate ends \cite[Section~4.1]{Donaldson-book},
  \cite{RS-strips}.
\end{enumerate}
The same holds on $W_{\pm,\infty}$, for pairs on $W_{-,\infty}\sqcup W_{+,\infty}$, and with
$\alpha$ replaced by the joined stationary connections $B_p\cup\alpha\cup B_q$ on the
slices $D_{i,\lambda}$ when applied to finite joined configurations.
\end{lemma}

\begin{proof}
On each mixed end $e$ choose a temporal gauge in which $V$ converges to a
stationary pair. The stationary limit is gauge equivalent to $\pi^*B_{q_e}$ by a
gauge transformation independent of $\theta$; composing with it, the limit is
exactly $\pi^*B_{q_e}$, and~\eqref{eq:end-decay} gives (ii) for this representative
$V^E_e$ on $\{\theta\ge\Theta_V-1\}$ once $\Theta_V$ is beyond the threshold of
Proposition~\ref{prop:growing-ends}, because $\delta<c$. On
$T\times[S_V-1,\infty)\times\RR$ let $V^T$ be the torus slice of
Proposition~\ref{prop:tail-decay}. On the overlap $\{\theta\ge\Theta_V-1,\ r\ge S_V-1\}$
both representatives are $\alpha$ plus a term that is small in $C^1$ and tends to
zero with $\theta$: the first because $B_{q_e}=\alpha$ on the collar, the second by
Proposition~\ref{prop:tail-decay}(i). The relative gauge transformation is even and
maps a connection close to $\alpha$ to one close to $\alpha$, so it is close to the
even stabilizer of $\alpha$, which is trivial; it is $\exp\varphi$ with $\varphi$ small, and
the torus-slice argument (fibrewise $d_\alpha\varphi$ and the base derivatives of $\varphi$
are controlled by the two connection differences) gives
$\|\varphi\|_{H^{m+1}_\delta(\{\theta\ge\Theta,\ r\ge S_V-1\})}\to0$ as $\Theta\to\infty$. Replace
$V^E_e$ on the end by $\exp(\chi(r)\varphi)^*V^E_e$, with $\chi(r)=0$ for $r\le S_V-1$ and
$\chi(r)=1$ for $r\ge S_V$; it agrees with $V^T$ for $r\ge S_V$ and with $V^E_e$ for
$r\le S_V-1$, and it still satisfies (ii). Finally extend over the core. The even
gauge transformation relating $V$ to these representatives on the outer region
$O=\{r\ge S_V\}\cup\{|\theta|\ge\Theta_V\}$ and on the original ends (in this section, the
ends of $W_\infty$ other than the torus region) lifts to the
determinant-one group, so the obstruction to extending it over the core $K$ lies in
$H^4(K,K\cap O;\ZZ)$, which vanishes because $K$ meets the free genus matching
faces; the normalization of the temporal gauge at each mixed end is even for the
same reason, the critical manifolds being quotients by the determinant-one
group. The extension changes $V$ only on compact sets disjoint from $\{r\ge S_V\}\cup\{\theta\ge\Theta_V\}$, after enlarging $S_V$ and
$\Theta_V$.
\end{proof}
...
ends; and the stationary operators have a gap and the decay of
Proposition~\ref{prop:growing-ends} holds uniformly along the family. Then the
...
Family compactness uses the uniform decay and the $\lambda$-independent constants
of Theorem~\ref{thm:torus-period}.
\end{proof}
...
gap, depending continuously on the width, so the gap and the decay are uniform
along the family.
\end{proof}
...
further invariant perturbation supported in the pieces, small enough not to
...
$Y'_\mathsf R$ or $D_{i,\lambda}$). Fix $m\ge4$. The weight $\tau$ of
...
\[
0<\delta\le\delta_T,\qquad\delta<\gamma/2,
\]
where $\delta_T$ is as in Proposition~\ref{prop:tail-decay} and $\gamma$ is the gap of
Proposition~\ref{prop:stationary-gap}, and with $\delta$ below the weights of the
original mixed ends in \cite[Definition~3.9]{DFL}. Constants $C$, $C_{\rm inv}$, $C_{\rm quad}$ and $r_1$ depend on
...
\begin{enumerate}[label=(\roman*)]
\item $P_\mathsf L$ is a configuration with the labels of $V$ (of the pair), equals $\alpha$ near
...
\begin{proof}
(i) is immediate from Lemma~\ref{lem:adapted}. (ii) Only the truncation band
...
\[
\mathcal F_{\mathsf L}(z)=\Bigl(d_{A_P}^*\zeta,\ \mathfrak M\bigl(\Psi_{\mathsf L}(z)\bigr)\Bigr)\in Y_\mathsf L .
\]
...
\item $\Psi_{\mathsf L}(z)$ satisfies the nonlinear matching, Lagrangian and flat-fibre
  conditions, and zeros of $\mathcal F_\mathsf L$ are solutions of the mixed equation.
...
the section and the parts of the mixed ends away from the torus collar. For the
converse, first invert the chart at fixed $p$: at the face by
...
(b) is immediate: the gauge-fixing component does not involve $p$, the corrections
...
\begin{proof}
By Proposition~\ref{prop:torus-inverse} and Lemma~\ref{lem:torus-quadratic}, the map
...
holds in the whole ball of fixed radius $r$. The derivative
...
the Coulomb condition on the decaying coordinate and the stationary fields $k_e$; since this
...
\emph{The mixed ends.} The kinetic energy of $U_j$ beyond $\Theta-1$ converges to that
of $V$, which is at most $Ce^{-2c(\Theta-1-\Theta_V)}$ by~\eqref{eq:end-decay}; for $\Theta$ large
it is below the threshold of Proposition~\ref{prop:growing-ends}, which is uniform
in $\mathsf L$. In a temporal gauge on $R_E^e$, normalized by a gauge transformation
independent of $\theta$ so that the limit is exactly $\pi^*B_{q_{j,e}}$, the
representative $U_j^E$ satisfies, by~\eqref{eq:end-decay} summed over unit
intervals with the weight $e^{\delta\theta}$ and $\delta<c$,
\[
\|U^E_j-\pi^*B_{q_{j,e}}\|_{H^m_\delta(R^e_E)}\le Ce^{\delta\Theta}E_j(\{\theta\ge\Theta-1\})^{1/2}
...
functions of Section~\ref{subsec:folded}, which are fixed with the splitting; the
...
Let $p_0$, $p_1$ be admissible presentations of $M_0$, $M_1$ with DFL-regular primary
...
and Theorem~\ref{thm:A} applies to $N_{p_i}$. The second statement is the case of the
identity morphism: on the gauge homology at $p$ and $q$ the identity of $\Ish(M)$ is the
gauge continuation $G(q,p)$, and $S(1_M)_{qp}=\Gamma(q,p)$ by Lemma~\ref{lem:descent}.
\end{proof}

The corollary applies wherever admissible presentations with DFL-regular primary data
exist. Their existence at the fixed metric data of a presentation is the transversality
question of Remark~\ref{rem:dfl-regular}. Where they exist, Theorem~\ref{thm:A} shows that
the vertex map at any admissible presentation of the same object is, on homology, the
reduction modulo two of the isomorphism of \cite{DFL} at such a presentation, composed
with the maps $G$ and $\Gamma$.
...
\item \cite[Section~4.1 with (4.3) and Lemma~4.4, Proposition~4.6, Section~4.2.1 with
  (4.16), Propositions~4.65, 4.69, 5.27 and~5.28]{DFL}: the cutoff maps $E_\tau$ and a
  metric in which the Lagrangian is totally geodesic, the Coulomb slice (used in
  Section~\ref{sec:torus} in a variant), the slice operator on a cylinder quintuple,
  additivity of energy and index under cutoff gluing, and exponential decay on mixed
  ends. Used in Section~\ref{sec:torus}. Hypotheses: nondegenerate or Morse--Bott ends
  as stated there.
...
\item \cite[Section~14.4]{Oh-book}: continuation with moving Lagrangian
  boundary conditions.
\item \cite[Propositions~3.8--3.10, Theorems~4.17 and~5.4, Section~5.3]{Donaldson-book}:
  additivity of the index under gluing along acyclic ends, weights and
  kernels, neck gluing with surjectivity, and gluing at index $-1$ in families;
...
\item Gluing along ordinary strip necks with transverse intersections is
  ...
  \cite[Section~4]{DFL-mixed}; monotonicity enters only through
