...
continuity, embedding, neighborhood-surjectivity, and orientation properties
...
augmented parametrix in mixed critical norms gives $O(\lambda^{1/3})$ bounds
for the projected splicing error and nonlinear correction, uniformly over the
family.  Model-coordinate modulation recovers the background and gluing
parameters; a two-sided spectral estimate on the conformal neck gives the
no-neck theorem needed for reverse gluing.  The obstruction section extends
over the natural cone family, and equivariant determinant-line excision
identifies the actual and virtual links as oriented cycles.
...
We give its Jacobi-polynomial formula; the zero-dimensional formula and the
associated low-degree Donaldson--Seiberg--Witten comparison follow as
corollaries.  The result is confined to one bubble and does not treat
collision strata, zero-spinor solutions, or the full Witten conjecture.
\end{abstract}
...
this construction the continuity, embedding, and neighborhood-surjectivity
properties and assigns them to a companion article.  In the one-bubble case,
these properties are stated in~\cite[Theorem~3.8]{FeehanLenessLevelOne} and
used in the oriented link calculation there.  The global version appears as
Hypothesis~7.8.1 in~\cite{FeehanLenessCobordism}.
...
The neighborhood theorem supplies every invocation of the deferred gluing
result in Sections~3--6 of~\cite{FeehanLenessLevelOne}.  We retain the
notation of that paper for the pairing.  Let $\mathfrak t'$ be a
...
\begin{equation}\label{eq:asd-dimension}
 d_a(\mathfrak t')=-2p_1(\mathfrak t')-\tfrac32(\chi+\sigma),
\end{equation}
and set
...
\begin{thmletter}[family level-one pairing; $=$
Theorem~\ref{thm:pairing}]\label{thm:intro-pairing}
...
\[
 c(X)=-\frac14(7\chi+11\sigma).
\]
Combining Theorem~\ref{thm:intro-pairing} with the published top-level
calculation, blow-up formulas, and superconformal-simple-type vanishing gives
the following precise consequence.

\begin{thmletter}[degree consequence; $=$
Corollary~\ref{cor:degree}]\label{thm:intro-degree}
...
\subsection{Strong and weak levels}
...
Under the hypotheses preceding Theorem~\ref{thm:intro-neighborhood}, there is
...
Write $\mathfrak S(q)=(v_0,\psi_0)$ and let $\chi$ be the outer spinor cutoff.
On $4R<d(x,\cdot)<8R$ the pair is $(A_0,\chi\Phi_0)$ and
...
\begin{equation}\label{eq:outer-residual}
 \bigl(\mathcal Q(\Phi_0)-\mathcal Q(\chi\Phi_0),
 (\chi-1)\psi_0+\rho(d\chi)\Phi_0\bigr).
\end{equation}
...
$\|d\chi\|_{L^2}\leq CR$; consequently the last term in
...
gauge as $\Gamma+\chi_m(A_0-\Gamma)$.  Then
\begin{equation}\label{eq:middle-curvature}
 F=\chi_mF_{A_0}+d\chi_m\wedge(A_0-\Gamma)
   +(\chi_m^2-\chi_m)(A_0-\Gamma)\wedge(A_0-\Gamma).
\end{equation}
The bounds $|A_0-\Gamma|\leq Cr$ and $|d\chi_m|\leq C/r$ show that
...
source
\[
 c(dt)(\partial_t\chi)r^{3/2}\Phi_{\mathrm E},
\]
...
complex bundle $\Xi\to N_{\mathfrak t',\mathfrak s}$.  The charge-one Dirac
...
transports, and inverse are equivariant and its fixed point is unique.  The
two common presentations of the residual circle action should not be mixed.
Before taking the effective circle, the diagonal presentation has weight two
on the normal and background-obstruction factors.  After passage to the
effective action, the corresponding Euler class is
...
reduction of~\cite[Section~8]{FeehanLenessIII}.  Three objects must be kept
distinct.  On a relatively compact positive-scale chart $U^+$ on which a
...
\begin{theorem}[family level-one pairing]\label{thm:pairing}
Under the assumptions preceding Theorem~\ref{thm:intro-pairing},
\begin{align*}
...
\end{align*}
where $b_0=2n$ and $a_0,a_1$ are as in Theorem~\ref{thm:pairing}.
\end{corollary}
...
Under the additional abundance, effectiveness, Seiberg--Witten-simple-type,
and characteristic-data assumptions in Theorem~\ref{thm:intro-degree},
...
pairing~\cite{FeehanLenessII}, Theorem~\ref{thm:pairing}, the Donaldson and
